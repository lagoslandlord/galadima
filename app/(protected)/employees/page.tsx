...
export default function EmployeesPage() {
  ...
  async function handleManageSave() {
    ...
    try {
      ...
      if (canChangeRole && manageForm.role !== manageTarget.role) {
        ...
        if (!roleJson.success)
          toast.error(
            roleJson.error || "Updated profile, but role change failed",
          );
      }
  ...
  async function handleResetPassword() {
    ...
    setResetting(true);
    try {
      const res = await fetch(`/api/users/${manageTarget._id}/reset-password`, {
        method: "PATCH",
      });
      const json = await res.json();
      if (!json.success) {
      ...
      setNewCredentials({
        ...
        employeeId: manageTarget.employeeId,
        temporaryPassword: json.temporaryPassword,
      });
      setManageTarget(null);
    } catch {
      ...
      <div className="page-header">
        <div>
          ...
          <p style={{ color: "var(--color-neutral-500)", marginTop: 4 }}>
            Everyone at Harmony Garden, by department.
          </p>
        </div>
        {canManage && (
          <button
                  ...
                  <tr key={emp._id}>
                    <td>
                      <Link
                        ...
                        style={{ fontWeight: 600, color: "var(--color-info)" }}
                      >
                        {emp.name}
                      </Link>
                    </td>
                    ...
                    <td>
                      <span
                        className={`badge ${emp.isActive ? "badge-approved" : "badge-rejected"}`}
                      >
        ...
        >
          <form onSubmit={handleCreate}>
            ...
            <p className="form-hint">
              A temporary password is generated automatically — you&apos;ll see
              it once, right after this is created.
            </p>
          </form>
        </Modal>
      )}
        ...
        >
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={(e) =>
                setManageForm({ ...manageForm, email: e.target.value })
              }
            />
            <p className="form-hint">
              Changing this changes their login email too.
            </p>
          </div>
          <div className="form-group">
            <label className="form-label">Phone</label>
            <input
              ...
              onChange={(e) =>
                setManageForm({ ...manageForm, phone: e.target.value })
              }
            />
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            {!canChangeRole && (
              <p className="form-hint">Only a Super Admin can change roles.</p>
            )}
          </div>
          <div className="form-group">
            <label
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setManageForm({ ...manageForm, isActive: e.target.checked })
                }
              />
              Active
            </label>
      ...
      {newCredentials && (
        ...
        >
          <p style={{ marginBottom: 12, fontSize: "0.9375rem" }}>
            Share this with <strong>{newCredentials.name}</strong> (
            {newCredentials.employeeId}) — it won&apos;t be shown again.
          </p>
